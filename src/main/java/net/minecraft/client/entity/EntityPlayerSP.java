...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        EventUpdate event = new EventUpdate(posX, posY, posZ, rotationYaw, rotationPitch, onGround, false);
        event.call();
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(e.getX(), e.getY(), e.getZ(), e.getYaw(), e.getPitch(), e.getOnGround()));
                }
                else if (flag2)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(e.getX(), e.getY(), e.getZ(), e.getOnGround()));
                }
                else if (flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(e.getYaw(), e.getPitch(), e.getOnGround()));
                }
                else
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer(e.getOnGround()));
                }
            }
            else
            {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, e.getYaw(), e.getPitch(), e.getOnGround()));
                flag2 = false;
            }
